...
const problemChartConfig = {
    type: 'bar',
    data: {
        labels: ['Time', 'Errors', 'Paper', 'Comms', 'Vision'],
        datasets: [{
    ...
    options: {
        ...
        scales: {
            y: { beginAtZero: true, max: 100, grid: { color: 'rgba(255, 255, 255, 0.1)' }, ticks: { color: 'rgba(255, 255, 255, 0.7)', font: { size: 10 } } },
...
const solutionChartConfig = {
    type: 'radar',
    data: {
        labels: ['Efficiency', 'Accuracy', 'Org', 'Collab', 'Insights'],
        datasets: [{
            label: 'Our HR System',
            data: [90, 95, 85, 88, 92],
    ...
    options: {
        ...
        maintainAspectRatio: false,
        scales: {
            r: { beginAtZero: true, max: 100, grid: { color: 'rgba(255, 255, 255, 0.1)' }, ticks: { display: false }, pointLabels: { color: 'rgba(255, 255, 255, 0.7)', font: { size: 10 } } }
        },
        plugins: { legend: { display: false } }
    }
};
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-[linear-gradient(135deg,_#5c6bc0_0%,_#3949ab_100%)] text-white">
      ...
      <div className="relative z-10 flex flex-col h-full p-4 md:p-8 lg:p-10">
        <h1 className="text-3xl md:text-5xl lg:text-6xl font-extrabold mb-1 md:mb-3 bg-gradient-to-r from-white to-indigo-100 text-transparent bg-clip-text">Problem & Solution</h1>
        <p className="text-base md:text-xl lg:text-2xl font-medium mb-4 md:mb-8 text-indigo-100">Transforming HR Management Through Digital Innovation</p>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-8 flex-grow">
          <div className="bg-white/10 backdrop-blur-md rounded-xl lg:rounded-2xl p-3 md:p-4 lg:p-6 flex flex-col transition-all duration-300 hover:-translate-y-1 hover:bg-white/15">
            <div className="flex items-center mb-2 md:mb-4">
              <i className="fas fa-exclamation-triangle text-xl md:text-3xl mr-2 md:mr-4 text-amber-300"></i>
              <h3 className="text-xl md:text-3xl font-semibold">The Problem</h3>
            </div>
            <ul className="space-y-2 text-xs md:text-base lg:text-lg">
            ...
            <div className="mt-auto h-32 md:h-48 lg:h-56 pt-4"><BaseChart config={problemChartConfig as any} /></div>
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-xl lg:rounded-2xl p-3 md:p-4 lg:p-6 flex flex-col transition-all duration-300 hover:-translate-y-1 hover:bg-white/15">
            <div className="flex items-center mb-2 md:mb-4">
              <i className="fas fa-lightbulb text-xl md:text-3xl mr-2 md:mr-4 text-amber-300"></i>
              <h3 className="text-xl md:text-3xl font-semibold">Our Solution</h3>
            </div>
            <ul className="space-y-2 text-xs md:text-base lg:text-lg">
              ...
              <li className="flex items-center"><i className="fas fa-calculator w-6 text-amber-300"></i><span>100% accurate payroll calculations</span></li>
            ...
            <div className="mt-auto h-32 md:h-48 lg:h-56 pt-4"><BaseChart config={solutionChartConfig as any} /></div>
          </div>
        </div>
      </div>
    </div>
