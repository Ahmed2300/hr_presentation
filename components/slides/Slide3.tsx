...
const ComponentCircle: React.FC<{ icon: string; title: string; desc: string; className?: string }> = ({ icon, title, desc, className }) => (
    <div className={`absolute w-36 h-36 rounded-full flex flex-col justify-center items-center bg-white/20 backdrop-blur-md border-2 border-white/40 shadow-lg z-10 transition-all duration-300 hover:scale-105 hover:bg-white/30 hover:shadow-2xl ${className}`}>
        <i className={`fas ${icon} text-4xl mb-2 text-amber-300`}></i>
        <div className="text-lg font-semibold text-center">{title}</div>
        <div className="text-xs text-center text-indigo-100 mt-1">{desc}</div>
    </div>
);

const FeatureCard: React.FC<{icon: string; title: string; desc: string}> = ({icon, title, desc}) => (
    <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 flex items-center transition-all duration-300 hover:-translate-y-1 hover:bg-white/15 hover:shadow-lg">
        <i className={`fas ${icon} text-3xl mr-4 text-amber-300 w-10 text-center`}></i>
        <div>
            <div className="text-lg font-semibold">{title}</div>
            <div className="text-sm text-indigo-100">{desc}</div>
        </div>
    </div>
)
    ...
    return (
        <div className="w-full h-full relative overflow-hidden bg-[linear-gradient(135deg,_#5c6bc0_0%,_#3949ab_100%)] text-white">
            ...
            <div className="relative z-10 flex flex-col h-full p-10 px-14">
                <h1 className="text-6xl font-extrabold mb-3 bg-gradient-to-r from-white to-indigo-100 text-transparent bg-clip-text">System Architecture</h1>
                <p className="text-2xl font-medium mb-8 text-indigo-100">How our HR/Manager System components interact seamlessly</p>
                <div className="flex flex-grow gap-8">
                    <div className="flex-[3] relative bg-white/10 backdrop-blur-md rounded-2xl p-5">
                        ...
                        <div className="absolute top-[30%] left-[35%] text-sm text-amber-300 bg-[#5c6bc0]/80 px-2 py-1 rounded-full">Requests</div>
                        <div className="absolute top-[30%] left-[55%] text-sm text-amber-300 bg-[#5c6bc0]/80 px-2 py-1 rounded-full">Data</div>
                        <div className="absolute top-[60%] left-[30%] text-sm text-amber-300 bg-[#5c6bc0]/80 px-2 py-1 rounded-full">Approvals</div>
                        <div className="absolute top-[60%] left-[55%] text-sm text-amber-300 bg-[#5c6bc0]/80 px-2 py-1 rounded-full">Analytics</div>
                    </div>
